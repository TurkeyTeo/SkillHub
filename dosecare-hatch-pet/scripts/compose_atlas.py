#!/usr/bin/env python3
"""Compose or normalize a DoseCare companion pet spritesheet atlas."""
    ...
    if source.size != (ATLAS_WIDTH, ATLAS_HEIGHT):
        ...
        if abs(source_ratio - ATLAS_ASPECT_RATIO) > 0.02:
            raise SystemExit(
                ...
                f"the DoseCare atlas ratio {ATLAS_ASPECT_RATIO:.3f}; got {source_ratio:.3f}. "
...
def main() -> None:
    ...
    parser.add_argument(
        "--resize-source",
        action="store_true",
        help="Resize a lower-resolution source atlas only when it already has the DoseCare atlas aspect ratio.",
    )
